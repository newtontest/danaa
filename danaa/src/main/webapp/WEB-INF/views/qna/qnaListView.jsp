					...
					<c:choose>
						<c:when	test="${ ((loginMember.id == 'admin11') or (loginMember.id == 'admin22')) or (loginMember.id == 'admin33') }">
							<c:url var="qrdetail" value="qrdetail.do">
                    		 	<c:param name="qna_no" value="${ q.qna_no }" />
                    		 	<c:param name="currentPage" value="${ currentPage }" />
           					 </c:url><a href="${ qrdetail }">&nbsp; &nbsp; └ <img src="resources/images/re.png">${ q.qna_title }<img src="resources/images/key.png"></a>
						</c:when>
							<c:when	test="${ ((loginMember.id != 'admin11') or (loginMember.id != 'admin22')) or (loginMember.id != 'admin33') }">
								<c:url var="rpwdCheck" value="moveReplyPwd.do">
                     				<c:param name="qna_no" value="${ q.qna_no }" />
                     				<c:param name="currentPage" value="${ currentPage }" />
            					</c:url><a href="${ rpwdCheck }">&nbsp; &nbsp; └ <img src="resources/images/re.png">${ q.qna_title }<img src="resources/images/key.png"></a>
           					</c:when>
						<c:otherwise>
							<c:url var="rpwdCheck" value="moveReplyPwd.do">
                     			<c:param name="qna_no" value="${ q.qna_no }" />
  ...
  <div class="adpage">
      ...
      <c:if test="${ (currentPage - 10) < startPage && (currentPage - 10) > 1 }">
         <c:url var="ubl2" value="qlist.do">
       	...
       	<a href="${ ubl2 }" class="ti-angle-left"></a>
      </c:if>
      <c:if test="${ !((currentPage - 10) < startPage && (currentPage - 10) > 1) }">
      ...
      <c:if test="${ (currentPage + 10) > endPage && (currentPage + 10) < maxPage }">
        ...
        <a href="${ ubl4 }" class="ti-angle-right"></a>
      ...
      <c:if test="${ currentPage >= maxPage }">
        ...
        <a href="${ ubl5 }" class="ti-angle-double-right"></a>
      </c:if>
      <!-- 글쓰기s -->
